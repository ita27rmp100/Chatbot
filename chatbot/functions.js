...
async function get_ai_res(prompt) {
    ...
    try {
        ...
        return "عذرا، حدث خطأ أثناء التواصل مع الخدمة الذكية.";
    }
}
...
async function FromDoc(question){
    try {
        ...
        const answer = await get_ai_res(`
            ...
            عذرا ، لا تتوفر لدينا معلومات كافية حول السؤال الذي طرحته ، يرجى التواصل مباشرة مع فريق الدعم عبر البريد الإلكتروني:
            <a href="mailto:rsbhousedz@gmail.com">اضغط هنا</a>
            أو عبر نموذج التواصل في الصفحة الرئيسية:
            <a href="/#contactus">اضغط هنا</a>
        ...
        console.error(err);
        return "عذرا، حدث خطأ أثناء الوصول إلى المعلومات.";
    }
}
async function Chatbot(qst) {
    ...
    if(firebaseResult.infirebase){
        ...
        console.log(answr)
        if(typeof answr === "string" && !answr.includes("عذرا")){
            configuration();
